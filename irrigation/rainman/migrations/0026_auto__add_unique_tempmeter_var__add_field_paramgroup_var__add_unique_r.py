...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding unique constraint on 'TempMeter', fields ['var']
        db.create_unique('rainman_tempmeter', ['var'])

        # Adding field 'ParamGroup.var'
        db.add_column('rainman_paramgroup', 'var',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=200),
                      keep_default=False)

        if not db.dry_run:
            for v in orm.ParamGroup.objects.all():
                v.var = v.name
                v.save()
        db.create_unique('rainman_paramgroup', ['var'])

        # Adding unique constraint on 'RainMeter', fields ['var']
        db.create_unique('rainman_rainmeter', ['var'])

        # Adding field 'Site.var'
        db.add_column('rainman_site', 'var',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=200),
                      keep_default=False)

        if not db.dry_run:
            for v in orm.Site.objects.all():
                v.var = v.name
                v.save()
        db.create_unique('rainman_site', ['var'])

        # Adding field 'Controller.var'
        db.add_column('rainman_controller', 'var',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=200),
                      keep_default=False)

        if not db.dry_run:
            for v in orm.Controller.objects.all():
                v.var = v.name
                v.save()
        db.create_unique('rainman_controller', ['var'])

        # Adding unique constraint on 'WindMeter', fields ['var']
        db.create_unique('rainman_windmeter', ['var'])

        # Adding unique constraint on 'SunMeter', fields ['var']
        db.create_unique('rainman_sunmeter', ['var'])

        # Adding unique constraint on 'Valve', fields ['var']
        db.create_unique('rainman_valve', ['var'])

    def backwards(self, orm):
        # Removing unique constraint on 'Valve', fields ['var']
        db.delete_unique('rainman_valve', ['var'])

        # Removing unique constraint on 'SunMeter', fields ['var']
        db.delete_unique('rainman_sunmeter', ['var'])

        # Removing unique constraint on 'WindMeter', fields ['var']
        db.delete_unique('rainman_windmeter', ['var'])

        # Removing unique constraint on 'RainMeter', fields ['var']
        db.delete_unique('rainman_rainmeter', ['var'])

        # Removing unique constraint on 'TempMeter', fields ['var']
        db.delete_unique('rainman_tempmeter', ['var'])

        # Deleting field 'ParamGroup.var'
        db.delete_column('rainman_paramgroup', 'var')

        # Deleting field 'Site.var'
        db.delete_column('rainman_site', 'var')

        # Deleting field 'Controller.var'
        db.delete_column('rainman_controller', 'var')
    ...
    models = {
        ...
        'rainman.log': {
            ...
            'timestamp': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 5, 14, 0, 0)', 'db_index': 'True'}),
