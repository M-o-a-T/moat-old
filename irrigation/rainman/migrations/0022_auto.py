...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding index on 'History', fields ['time']
        db.create_index('rainman_history', ['time'])

        # Adding index on 'Level', fields ['time']
        db.create_index('rainman_level', ['time'])

        # Adding index on 'ValveOverride', fields ['start']
        db.create_index('rainman_valveoverride', ['start'])

        # Adding index on 'GroupOverride', fields ['start']
        db.create_index('rainman_groupoverride', ['start'])

        # Adding index on 'Schedule', fields ['start']
        db.create_index('rainman_schedule', ['start'])

        # Adding index on 'Log', fields ['timestamp']
        db.create_index('rainman_log', ['timestamp'])

        # Adding index on 'GroupAdjust', fields ['start']
        db.create_index('rainman_groupadjust', ['start'])

        # Adding index on 'Valve', fields ['time']
        db.create_index('rainman_valve', ['time'])


    def backwards(self, orm):
        
        # Removing index on 'Valve', fields ['time']
        db.delete_index('rainman_valve', ['time'])

        # Removing index on 'GroupAdjust', fields ['start']
        db.delete_index('rainman_groupadjust', ['start'])

        # Removing index on 'Log', fields ['timestamp']
        db.delete_index('rainman_log', ['timestamp'])

        # Removing index on 'Schedule', fields ['start']
        db.delete_index('rainman_schedule', ['start'])

        # Removing index on 'GroupOverride', fields ['start']
        db.delete_index('rainman_groupoverride', ['start'])

        # Removing index on 'ValveOverride', fields ['start']
        db.delete_index('rainman_valveoverride', ['start'])

        # Removing index on 'Level', fields ['time']
        db.delete_index('rainman_level', ['time'])

        # Removing index on 'History', fields ['time']
        db.delete_index('rainman_history', ['time'])
