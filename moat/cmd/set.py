	...
	def do(self,args):
		...
		if self.delete:
			for a in args:
				...
				try:
					self.root.sync(self.root.etcd.delete('/'+pa, recursive=True,**kw))
				except etcd.EtcdCompareFailed:
					print("Bad modstamp: /"+a, file=sys.stderr)
			...
			for a in args:
				...
				try:
					...
					if self.append:
						kw['append']=True
					r = self.root.sync(self.root.etcd.set('/'+pa, prev=None, **kw))
					if self.append:
						print(r.key.rsplit('/',1)[1])
				except etcd.EtcdCompareFailed:
					print("Bad modstamp: "+a, file=sys.stderr)
				except etcd.EtcdAlreadyExist:
					print("Entry exists: "+a, file=sys.stderr)
